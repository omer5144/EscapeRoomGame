from .scene import Scene
from .scene import Object
from .first_room import FirstRoom
from .second_room import SecondRoom
from .last_room import LastRoom
from .left_team_room import LeftTeamRoom
from .right_team_room import RightTeamRoom
from .middle_team_room_back import MiddleTeamRoomBack
from .ptouch_note import (PtouchNoteFirst, PtouchNoteSecond, PtouchNoteThird, PtouchNoteFourth, PtouchNoteFifth,
                          PtouchNoteSixth)

PTOUCH_SCENES = [PtouchNoteFirst, PtouchNoteSecond, PtouchNoteThird, PtouchNoteFourth, PtouchNoteFifth,
                 PtouchNoteSixth]
